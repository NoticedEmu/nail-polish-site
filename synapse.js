...
function synFetchWithTimeout(url,options={},timeoutMs=7000){const controller=new AbortController();const timer=setTimeout(()=>controller.abort(),timeoutMs);return fetch(url,{...options,signal:controller.signal}).finally(()=>clearTimeout(timer))}
async function fetchMCNSSkeleton(id,name=''){const key=String(id);if(mcnsSkeletonCache.has(key))return mcnsSkeletonCache.get(key);const r=await synFetchWithTimeout(MCNS_SOURCE+key+'.swc',{},7000);if(!r.ok)throw Error('MaleCNS skeleton '+key+' unavailable ('+r.status+')');const sk=parseSWC(await r.text(),{id:key,name:name||key});mcnsSkeletonCache.set(key,sk);return sk}
async function loadMCNS(){
  const status=syn$('#mcnsStatus');
  // Connectivity is a local site asset and is the only dependency Ask the Fly needs.
  // Load it first so a slow/blocked remote SWC request can never hold the interaction hostage.
  try{
    const r=await fetch('./mcns_connectome.json',{cache:'no-store'});
    if(!r.ok)throw Error('mcns_connectome.json unavailable ('+r.status+')');
    const graph=await r.json();
    if(!graph||!Array.isArray(graph.edges)||!graph.edges.length)throw Error('mcns_connectome.json has no edges');
    mcnsGraph=graph;
    status.textContent=`MaleCNS v1.0 connectivity ready • ${graph.edges.length} weighted connections • loading 3D morphology…`;
  }catch(e){
    console.error('MaleCNS connectivity load failed:',e);
    status.textContent='MaleCNS connectivity could not be loaded. Check mcns_connectome.json.';
    status.classList.add('mcns-error');
    return;
  }

  // Morphology is enhancement-only. Fetch independently and with a timeout.
  const results=await Promise.allSettled([
    fetchMCNSSkeleton('12781','DNge104_R'),
    fetchMCNSSkeleton('556329','DNge104_L')
  ]);
  mcnsSkeletons=results.filter(x=>x.status==='fulfilled').map(x=>x.value);
  if(mcnsSkeletons.length){
    mcnsLoaded=true;
    status.classList.remove('mcns-error');
    status.textContent=`Real MaleCNS v1.0 • ${mcnsGraph.edges.length} weighted connections • ${mcnsSkeletons.length}/2 reference morphologies loaded • CC BY`;
    if(synCurrent){synResizeCanvas();synMorphLayout=buildSynapseMorphLayout();synRenderNodes()}
    requestAnimationFrame(drawFlyNeuron);
  }else{
    // Ask the Fly remains fully functional using the real local connectivity graph.
    mcnsLoaded=false;
    status.textContent=`MaleCNS v1.0 connectivity ready • ${mcnsGraph.edges.length} weighted connections • 3D morphology unavailable`;
    console.warn('MaleCNS SWC morphology did not load; connectivity remains active.',results);
  }
}
